...
MAX_DATA_POINTS = 300
TURNS_RATIO = 1.0 / 20000

# =======================
# Clase principal
# =======================
class SensorMonitor:
    """Maneja buffers de datos y conexión al encoder QSB."""
    def __init__(self):
    ...
    def _load_thresholds(self):
        try:
            ...
            default = { g:{d:{"min":0,"max":100} for d in ["3/4","7/8","1","1 1/8"]}
    ...
    def monitor_clear_encoder(self):
        """Resetea el contador del encoder (vuelve a 0)."""
        if self.ser is not None:
            clear_encoder(self.ser)

    def read_sensor(self, com_port):
        """
        Si no hay COM seleccionado → genera datos de prueba.
        Si hay COM seleccionado → lee desde el encoder real.
        """
        if com_port is None:
            # Modo de simulación
            import random
        ...
        return encoder_val * TURNS_RATIO
...
def create_plot(df, thresholds, grade, diameter):
    ...
    fig.update_layout(
        title=f'Monitor de Sensor - {grade} {diameter}\"',
        xaxis_title='Tiempo',
...
# =======================
def main():
    st.set_page_config(page_title="Monitor de Sensor", layout="wide")
    st.title("Monitor de Sensor en Tiempo Real")

    # Inicialización de variables de sesión
    if 'monitor' not in st.session_state:
        st.session_state.monitor = SensorMonitor()
    ...
    with st.sidebar:
        ...
        if available_ports:
            ...
            if selected_port_info == "Ninguno (Datos de prueba)":
                ...
                if com != st.session_state.selected_com:
                    # Cambio de puerto: limpiar estado
                    st.session_state.selected_com = com
                    st.session_state.ser_configured = False
                st.success(f"Conexión exitosa al puerto : {com}")
        ...
        with col1:
            grade = st.selectbox(
                "Grado de Acero",
                options=["D","MMS", "UHS", "ARHS", "ARCS"], # Cambiar por listado de opciones de thresholds.json
                index=0
            )
        with col2:
            diameter = st.selectbox(
                "Diámetro",
                options=["3/4", "7/8", "1", "1 1/8"],
                index=0
            )
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Mínimo", f"{thr['min']}mm")
        with col2:
            st.metric("Máximo", f"{thr['max']}mm")
        ...
        with col2:
            if st.button("⏹️ Pause", disabled=not st.session_state.is_running):
                ...
                st.rerun()
        with col3:
            if st.button("🗑️ Clean", disabled=st.session_state.is_running):
                st.session_state.monitor.monitor_clear_encoder()
                st.session_state.monitor.clear()
                st.rerun()

        st.markdown("---")
        csv_data = st.session_state.monitor.df().to_csv(index=False).encode('utf-8')
        st.download_button(
            label="💾 Guardar CSV",
            data=csv_data,
        ...
        st.markdown("---")
        st.caption(f"Frecuencia de muestreo: {SAMPLING_FREQUENCY} Hz")
    ...
    with cols[1]:
        if not df.empty:
            ...
            st.metric("Máximo", f"{df['value'].max():.2f} mm")
            ...
            # Calcular pendientes (mm/s) para cada segmento
    ...
    if st.session_state.is_running:
        val = st.session_state.monitor.read_sensor(st.session_state.selected_com)
...
UPDATE_INTERVAL = 1.0 / SAMPLING_FREQUENCY
MAX_DATA_POINTS = 500
TURNS_RATIO = 1.0 / 10000 # Relación de vueltas para 1", Calibrar con este parametro
...
class EncoderMonitor:
    ...
    def read_sensor(self, com_port,diameter="1"):
        ...
        encoder_val = read_encoder(self.ser)  # Encoder real
        displacement = encoder_val * TURNS_RATIO
        if diameter == "3/4":
    ...
    def disconnect_serial(self) -> None:
        """Desconecta el encoder de forma segura."""
        try:
            if self.ser is not None:
                if self.ser.is_open:
                    self.ser.close()
...
def main():
    ...
    with st.sidebar:
        ...
        with col2:
            if st.button("🔌 Liberar puerto COM",disabled=not st.session_state.ser_configured_button_enable):
                ...
                st.session_state.is_running = False   # 🔑 frena la lectura
                time.sleep(0.1)                       # pequeño delay para que termine el loop
                st.session_state.monitor.disconnect_serial()
                st.session_state.ser_configured_button_enable = False
        ...
        # 📁 Generar CSV solo si no existe o si cambió el DataFrame
        ...
        try:
            ...
            # Opcional: mensaje visual en la app
    ...
    with cols[1]:
        if not df.empty:
            ...
            #filtered_deltas = time_diffs[time_diffs < 1.0]
            #if not filtered_deltas.empty:
            #    avg_freq = 1.0 / filtered_deltas.mean()
            #    st.metric("Freq. muestreo promedio", f"{avg_freq:.2f} Hz")
